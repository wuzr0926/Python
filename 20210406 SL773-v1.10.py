...
def Myz():
  ...
  lst = [Myz_v,100*Myz_v,R,K,Ly_v,lamda,lamda_x,m,a,Sy_v,B,E,Tv,S,A]
  return lst
...
def Myd():
  ...
  lst = [Myd_v,Myd_v*100,R,Kyd_v,K,N,Ly_v,lamda,lamda_x,m,a,Sy_v,B,E,Tv,S,A]
  return lst
...
def Mkw():
  ...
  lst = [Mkw_v,Mkw_v*100,Gkw,SIL,CLA,p,R,Lkw,lamda,lamda_x,a,Skw,S,A]
  return lst
...
def Mky():
  ...
  lst = [Mky_v,Mky_v*100,Gky,SIL,CLA,p,Fky,W,Lky,lamda,lamda_x,a,Sky,S,A]
  return lst
...
def Mdw():
  ...
  lst = [Mdw_v,100*Mdw_v,Gdw,a1,b1,delta,R,X,Ldw,lamda,lamda_x,f1,a,Sdw,d1,S,A]
  return lst
...
def Mdy():
  ...
  lst = [Mdy_v,100*Mdy_v,Gdy,a2,b2,delta,Fdy,W,Ldy,lamda,lamda_x,f2,a,Sdy,d2,S,A]
  return lst
...
##综合计算
def cal():
  district = input("请输入计算单元所在工程分区名称：")
  begin = int(input("请输入数字选择对应计算方法：\n\t1.植被破坏型\n\t2.地表翻扰型\n\t3.上方无来水开挖\n\t4.上方有来水开挖\n\t5.上方无来水堆积\n\t6.上方有来水堆积"))
  if begin == 1:
    variable = ["土壤流失量","土壤侵蚀模数","降雨侵蚀力因子","土壤可蚀性因子","坡长因子","投影坡长","斜坡坡长","坡长指数","坡度","坡度因子","植被覆盖因子","工程措施因子","耕作措施因子","计算单元宽度","计算单元水平投影面积(㎡)"]
    variable_v = Myz()
  elif begin == 2:
    variable_v = Myd()
    variable = ["土壤流失量","土壤侵蚀模数","降雨侵蚀力因子","翻扰后土壤可蚀性因子","土壤可蚀性因子","增加系数","坡长因子","投影坡长","斜坡坡长","坡长指数","坡度","坡度因子","植被覆盖因子","工程措施因子","耕作措施因子","计算单元宽度","计算单元水平投影面积(㎡)"]
  elif begin == 3:
    variable_v = Mkw()
    variable = ["土壤流失量","土壤侵蚀模数","无来水开挖面土石质因子","粉粒含量","黏粒含量","土体密度","降雨侵蚀力因子","无来水开挖面坡长因子","投影坡长","斜坡坡长","坡度","无来水开挖面坡度因子","计算单元宽度","计算单元水平投影面积(㎡)"]  
  elif begin == 4:
    variable_v = Mky()
    variable = ["土壤流失量","土壤侵蚀模数","有来水开挖土质因子","粉粒含量","黏粒含量","土体密度","有来水开挖面径流冲蚀力因子","上方单宽次来水总量","有来水开挖面坡长因子","投影坡长","斜坡坡长","坡度","有来水开挖面坡度因子","计算单元宽度","计算单元水平投影面积(㎡)"]
  elif begin == 5:
    variable_v = Mdw()
    variable = ["土壤流失量","土壤侵蚀模数","土石质因子","土石质因子系数","土石质因子系数","土体砾石含量","降雨侵蚀力因子","形态因子","坡长因子","投影坡长","斜坡坡长","坡长因子系数","坡度","坡度因子","坡度因子系数","计算单元宽度","计算单元水平投影面积(㎡)"]
  else:
    variable_v = Mdy()
    variable = ["土壤流失量","土壤侵蚀模数","有来水堆积土石质因子","有来水堆积土石质因子系数","有来水堆积土石质因子系数","土体砾石含量","有来水堆积体径流冲蚀力因子","上方单宽次来水总量","有来水堆积体坡长因子","投影坡长","斜坡坡长","有来水堆积体坡长因子系数","坡度","有来水堆积坡度因子","有来水堆积体坡度因子系数","计算单元宽度","计算单元水平投影面积(㎡)"]  
  import xlsxwriter
 ...
 # mychart = workbook.add_chart({'type':'column'})
  sheet1.write_row('A1',title,bold)
  sheet1.write_column('A2',data[0])
  sheet1.write_column('C2',data[1])
  workbook.close()
